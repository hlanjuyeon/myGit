...
$(function() {
	  ...
	  var request = $.ajax({
	    url: "/calendar/monthEdit",  // 데이터를 조회하는 API 엔드포인트 주소
	    method: "GET",
	    data: {no:no},
	    dataType: "json"
	  });
	  ...
	  request.done(function(data) {
	    ...
	    document.getElementById('title').value = data[0].titleModal;
	    document.getElementById('start').value = data[0].startModal;
	    document.getElementById('end').value = data[0].endModal;
	    document.getElementById('location').value = data[0].location;
	    document.getElementById('content').value = data[0].content;
	    document.getElementById('attendee').value = data[0].attendee;
	    document.getElementById('start').value = data[0].startModal;
		document.getElementById('end').value = data[0].endModal;
		/* document.querySelector('.writer').innerText = data[0].writer; */
	    
	    const btn_open = document.querySelector('.open');
		const btn_notopen = document.querySelector('.notopen');
	    
		if(data[0].privacy == 'open') {
			document.querySelector('.privacy-btn-select').innerText = '전체공개';
			document.querySelector('.privacy-btn-select').style.backgroundColor = 'rgb(233, 233, 233)';
		   	btn_open.style.backgroundColor = 'rgb(224, 236, 258)';
		   	btn_open.name = 'select_on';
		} else if(data[0].privacy == 'notopen') {
			document.querySelector('.privacy-btn-select').innerText = '비공개';
			document.querySelector('.privacy-btn-select').style.backgroundColor = 'rgb(233, 233, 233)';
		   	btn_notopen.style.backgroundColor = 'rgb(224, 236, 258)';
		   	btn_notopen.name = 'select_on';
		}
		
		switch(data[0].color) {
		case "gray":
			document.querySelector('.color-btn-select').innerHTML = document.querySelector('.btn-gray').innerHTML;
			break;
		case "red":
			document.querySelector('.color-btn-select').innerHTML = document.querySelector('.btn-red').innerHTML;
			break;
		case "orange":
			document.querySelector('.color-btn-select').innerHTML = document.querySelector('.btn-orange').innerHTML;
			break;
		case "hotpink":
			document.querySelector('.color-btn-select').innerHTML = document.querySelector('.btn-pink').innerHTML;
			break;
		case "green":
			document.querySelector('.color-btn-select').innerHTML = document.querySelector('.btn-green').innerHTML;
			break;
		case "blue":
			document.querySelector('.color-btn-select').innerHTML = document.querySelector('.btn-blue').innerHTML;
			break;
		case "mediumpurple":
			document.querySelector('.color-btn-select').innerHTML = document.querySelector('.btn-purple').innerHTML;
			break;
		}	
	...
	<div class="add" tabindex="-1" role="dialog" aria-labelledby="exampleaddLabel" aria-hidden="true">
        <div class="add-dialog" role="document">
            <form action="/calendar/update" method="get">
                ...
                <div class="add-header">
                	...
                	<input type="text" class="add-title" name="title" id="title" placeholder="제목 추가">
                </div>
                <div class="add-body">
                    <div class="form-group">
                        ...
                        <div>
                        	...
                        	<input type="text" class="add-location" id="location" name="location" placeholder="위치 추가">
                        </div>
                        <div>
                        	<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-person-fill" viewBox="0 0 16 16">
  								<path d="M3 14s-1 0-1-1 1-4 6-4 6 3 6 4-1 1-1 1H3Zm5-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z"/>
							</svg>
                        	<input type="text" class="add-attendee" id="attendee" name="attendee" placeholder="참석자 추가">
                        </div>
                        <div>
                        ...
                        <div>
	                        ...
	                        <textarea wrap=on class="add-content" id="content" name="content" placeholder="내용 추가"></textarea>
                        </div>
                    </div>
                </div>
